...
class AddressController {
  ...
  async addAddress(req, res, next) {
    try {
      ...
      const { title, address_line, city, region, postal_code, is_default } = req.body;

      if (!address_line || !city) {
        return res.status(400).json({ error: "address_line və city mütləqdir" });
      }
      ...
      const newAddress = await Address.create({
        user_id, title, address_line, city, region, postal_code, is_default: is_default || false
      });
